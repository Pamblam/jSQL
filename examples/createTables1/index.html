...
<html>
    ...
    <body>
		...
		<script>
			...
			jSQL.createTable("ceTable", ['a', 'b', 'c', 'd', 'e', 'f', 'g'], [
				[12,34,56,78],
				[23,45,67,89,78,78],
				[7], {"a":1, "b":2, "g":3}
			]);
			
			// Using a combination, implicit column names
			jSQL.createTable("ciTable", [
				[12,34,56,78],
				[23,45,67,89,78,78],
				[23,45,67,89,78,78,"sdfasdf"],
				[7], {"a":1, "b":2, "c":3},
				{"d":1, "e":2, "f":3}
			]);
			...
			echo("<h3>Example 4</h3><p>Created `ciTable` from arrays and objects, with implicit column names</p>");
			echo("<p>There are "+jSQL.tables.ciTable.data.length+" total rows in `ciTable`</p>");
			var results = jSQL.query('SELECT * FROM `ciTable`').execute().fetchAll();
			echo(makeTable(results));
